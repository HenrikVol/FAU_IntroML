...
def add_salt_and_pepper_noise(image: np.ndarray, salt_prob: float = 0.01, pepper_prob: float = 0.01) -> np.ndarray:
    ...
    # One random value per pixel position
    random_values = np.random.random(image.shape[:2])

    # Pepper noise: turn some pixels black
    noisy_image[random_values < pepper_prob] = 0

    # Salt noise: turn some pixels white
    noisy_image[random_values > 1 - salt_prob] = 255
...
def add_uniform_noise(image: np.ndarray, low: float = -20.0, high: float = 20.0) -> np.ndarray:
    ...
    # 2nd Add the noise to the image
    noisy_image = image.astype(np.float32) + noise
    
    # 3rd Clip values in the valid image range and convert back to uint8
    noisy_image = np.clip(noisy_image, 0, 255).astype(np.uint8)
